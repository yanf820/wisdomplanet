...
public class BaseProcessor implements IProcessor {
    ...
    public void activate(IProcessActivateContext activateContext) throws Exception {
        ...
        this.processMethods = new HashMap<Class<?>, Method>();
        ...
        for(Method method : methods){
            ...
            if(mName.startsWith(PROCESS_METHOD_PREFIX)){
                ...
                if(pTypes!=null && pTypes.length==3){
                    if(IProcessInput.class.isAssignableFrom(pTypes[1]) && IProcessOutput.class.isAssignableFrom(pTypes[2])){
        ...
        if(input.hasInputObject()) {
            ...
            if(!handled) {
                ...
                if (method != null) {
                    ...
                    throw new Exception("Can not found the process method for input object(" + inputObject + ") in Processor(" + this.getClass() + ")");
                }
            }
        }else{
    ...
    protected void processByMethod(Method method, Object inputObject, IProcessInput input, IProcessOutput output) throws Exception{
